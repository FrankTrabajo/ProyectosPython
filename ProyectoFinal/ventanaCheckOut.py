...
class CheckOut:
    ...
    def __init__ (self, ventana_inicio):
        ...
        self.ventana_out = Toplevel()
        self.ventana_out.title("Hostal Cruz Sol")
        self.ventana_out.resizable(1, 1)  # Activa la redimension de la ventana. Para desactivarla (0,0)
        self.ventana_out.wm_iconbitmap('recursos/cruzSol.ico')
        self.ventana_out.geometry("1065x450+240+120")
        ...
        titulo = Label(self.ventana_out, text="CHECKOUT", font=('Calibri', 16, 'bold'), fg='#000080')
        titulo.grid(row=0, column=0)

        clientes_out = LabelFrame(self.ventana_out, text="CLIENTES", font=('Calibri', 16, 'bold'))
        clientes_out.grid(row=1, column=0, padx=10)


    def listar_clientes(self):

        for widget in self.ventana_out.winfo_children():
            widget.destroy()

        # Etiquetas de encabezado
        self.etiqueta_doc_cli = Label(self.ventana_out, text="Documento", padx=55, pady=5, font=('Calibri', 11, 'bold'))
        self.etiqueta_doc_cli.grid(row=0, column=0)

        self.etiqueta_nombre_cli = Label(self.ventana_out, text="Nombre", padx=55, pady=5, font=('Calibri', 11, 'bold'))
        self.etiqueta_nombre_cli.grid(row=0, column=1)

        self.etiqueta_apellido_cli = Label(self.ventana_out, text="Apellidos", padx=55, pady=5, font=('Calibri', 11, 'bold'))
        self.etiqueta_apellido_cli.grid(row=0, column=2)

        self.etiqueta_hab_cli = Label(self.ventana_out, text="Habitacion", padx=55, pady=5, font=('Calibri', 11, 'bold'))
        ...
        for i, cliente in enumerate(self.clientes, start=2):
            ...
            doc_cli = Label(self.ventana_out, text=documento, padx=55, pady=5, font=('Calibri', 11))
            doc_cli.grid(row=i, column=0)

            nombre_cli = Label(self.ventana_out, text=nombre, padx=55, pady=5, font=('Calibri', 11))
            nombre_cli.grid(row=i, column=1)

            apellido_cli = Label(self.ventana_out, text=apellidos, padx=55, pady=5, font=('Calibri', 11))
            apellido_cli.grid(row=i, column=2)

            hab_cli = Label(self.ventana_out, text=habitacion, padx=55, pady=5, font=('Calibri', 11))
            ...
            btn_elim = Button(self.ventana_out, text="Check Out", padx=55, pady=5, font=('Calibri', 11), command=eliminar_funcion)
